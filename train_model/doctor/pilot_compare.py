...
def report_combined_ranking():
    """Baseline/CLHAA were carried forward to full-dataset training over Fajardo/Benitez.
    This makes that decision's evidence explicit and reproducible: sum each pipeline's best
    validation CER across CRNN and TrOCR (both already on the same 0-1 CER scale), and rank
    by the total - lowest two win. Written next to the two per-model summaries so the
    combined ranking behind the choice has a checked-in artifact, not just documentation."""
    results_dir = os.path.join(_HERE, "..", "..", "data", "doctor", "results")
    crnn = pd.read_csv(os.path.join(results_dir, "crnn_pilot_summary.csv"))[["pipeline", "best_val_cer"]]
    trocr = pd.read_csv(os.path.join(results_dir, "trocr_pilot_summary.csv"))[["pipeline", "best_val_cer"]]
    combined = crnn.merge(trocr, on="pipeline", suffixes=("_crnn", "_trocr"))
    combined["total_best_val_cer"] = combined["best_val_cer_crnn"] + combined["best_val_cer_trocr"]
    combined = combined.sort_values("total_best_val_cer").reset_index(drop=True)

    print("\n" + "=" * 60)
    print("[pilot] Combined ranking (CRNN + TrOCR best validation CER)")
    print("=" * 60)
    print(combined.to_string(index=False))

    out_path = os.path.join(results_dir, "pilot_combined_ranking.csv")
    combined.to_csv(out_path, index=False)
    print(f"[pilot] Saved to: {out_path}")
