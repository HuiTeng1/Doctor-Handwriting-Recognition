...
def run_trocr_all(train_df=None, val_df=None, ckpt_root=None, results_path=None, eval_num_beams=None, pipelines=None):
    ...
    def finetune_one(pipeline_name, train_df, val_df):
        ...
        # clhaa is hard-capped at epoch 11 - time constraints (each TrOCR full-data epoch
        # takes ~2.4h including the cooldown below). baseline's own epoch 9->11 run showed
        # eval_cer was already past its best (epoch 9: 11.84%) and regressing by epoch 11
        # (12.46%), so capping clhaa at the same epoch 11 mirrors a point where baseline
        # itself had already stopped improving - not an arbitrary cutoff.
        max_epochs = 11 if pipeline_name == "clhaa" else 1000
