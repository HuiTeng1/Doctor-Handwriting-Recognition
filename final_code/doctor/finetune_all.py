...
Only touches data/Doctor_clean/train.csv + val.csv, never test.csv.
...
SEED = 42
PIPELINES = ["baseline"]  # clhaa/fajardo/benitez have already been compared via the 15% pilot + test.csv acceptance check, baseline won, so the full run only needs to run baseline
...
def run_crnn_all(train_df=None, val_df=None, ckpt_root=None, results_path=None, pipelines=None):
    ...
    base_ckpt_dir = os.path.join(_HERE, "..", "normal", "checkpoint", "crnn")
    ckpt_root = ckpt_root or os.path.join(_HERE, "checkpoints")
    results_path = results_path or os.path.join(_HERE, "results", "crnn_finetune_summary.csv")
    ...
    def finetune_one(pipeline_name, base_ckpt_path, train_df, val_df):
        ...
        return {
            "pipeline": pipeline_name, "best_val_cer": best_cer,
            "stopped_epoch": trainer.current_epoch, "best_ckpt": checkpoint_callback.best_model_path,
        }
...
# TrOCR
# ======================================================================
def run_trocr_all(train_df=None, val_df=None, ckpt_root=None, results_path=None, eval_num_beams=None, pipelines=None):
    import torch
    ...
    base_trocr_dir = os.path.join(_HERE, "..", "normal", "checkpoint", "trocr")
    ckpt_root = ckpt_root or os.path.join(_HERE, "checkpoints_trocr")
    results_path = results_path or os.path.join(_HERE, "results", "trocr_finetune_summary.csv")
    max_target_length = 32
    batch_size = 4          # 4GB VRAM (RTX3050), same as Stage 1. No need to leave room for Chrome anymore, use it all
    grad_accum_steps = 4
    learning_rate = 5e-5

    def filter_trocr_empty(df, name):
        df = df.copy()
        df["text"] = df["text"].astype(str).str.strip()
        before = len(df)
        ...
        if dropped:
            print(f"[trocr] {name}.csv: dropping {dropped}/{before} rows (empty label)")
        return df

    def load_doctor_split(name):
        return filter_trocr_empty(pd.read_csv(os.path.join(DOCTOR_CLEAN_DIR, f"{name}.csv")), name)
    ...
    def finetune_one(pipeline_name, train_df, val_df):
        ...
        training_args = Seq2SeqTrainingArguments(
            ...
            num_train_epochs=1000,      # a soft cap - actually stopped by EarlyStoppingCallback,
        ...
        class CooldownCallback(TrainerCallback):
            """Sleeps in place for 15 minutes after every epoch, letting the GPU actually
            sit idle and cool down before the next epoch starts. Doesn't restart the
            process or reload the model/optimizer, avoiding the OOM risk that comes with
            reloading optimizer.pt on every resume (hit that once already tonight)."""
            COOLDOWN_SECONDS = 15 * 60

            def on_epoch_end(self, args, state, control, **kwargs):
                import time
                print(f"[trocr:{pipeline_name}] epoch finished, resting {self.COOLDOWN_SECONDS // 60} minutes to cool the GPU down...")
                time.sleep(self.COOLDOWN_SECONDS)
                print(f"[trocr:{pipeline_name}] rest over, continuing training")
        ...
        return {
            ...
            "best_val_wer": metrics.get("eval_wer"), "final_model_dir": final_dir,
        }

    if train_df is None:
        train_df, val_df = load_doctor_split("train"), load_doctor_split("val")
    else:
        train_df, val_df = filter_trocr_empty(train_df, "train"), filter_trocr_empty(val_df, "val")
