"""
baseline vs clhaa comparison, run on a stratified-sampled 15% subset of train+val,
for both CRNN and TrOCR.

======================================================================
Background (full rationale is in finetune_all.py's top docstring, only the conclusions
are here):
======================================================================
- fajardo/benitez already excluded: three independent data points - CRNN full finetune,
  CRNN 15% pilot (old version, before the letterbox fix), and zero-shot (CRNN+TrOCR,
  Normal model not finetuned) - all show them clearly worse.
- baseline vs clhaa is a small gap that's hard to measure precisely; it has to be tested
  with a real finetune, not guessed from existing data/zero-shot/theory.
...
- Decision layering: this step only uses train(15%)+val(15%), test.csv is untouched. val
  is only used for early stopping + picking the best checkpoint + comparing baseline vs
  clhaa; test.csv is reserved until after this step picks a winner, at which point a
  single inference pass on the winner (in a later script, not yet run) produces the
  actual final score.
======================================================================

Stratification is based on the 3 source families recognized in code/doctor_preprocessing.py:
...
Reuses run_crnn_all/run_trocr_all from finetune_all.py (with PIPELINES set to
["baseline","clhaa"]), just passing in this script's own train_df/val_df plus separate
checkpoint/results directories (checkpoints_pilot / checkpoints_trocr_pilot) so they
...
def build_pilot_splits():
    ...
    pilot_train.to_csv(os.path.join(_HERE, "pilot_train.csv"), index=False)
    pilot_val.to_csv(os.path.join(_HERE, "pilot_val.csv"), index=False)
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("Stage 1/2: CRNN pilot - baseline vs clhaa")
    print("=" * 60)
    run_crnn_all(
        pilot_train, pilot_val,
        ckpt_root=os.path.join(_HERE, "checkpoints_pilot"),
        results_path=os.path.join(_HERE, "results", "crnn_pilot_summary.csv"),
        pipelines=PILOT_PIPELINES,
    )

    print("\n" + "=" * 60)
    print("Stage 2/2: TrOCR pilot - baseline vs clhaa")
    print("=" * 60)
    run_trocr_all(
        pilot_train, pilot_val,
        ckpt_root=os.path.join(_HERE, "checkpoints_trocr_pilot"),
        results_path=os.path.join(_HERE, "results", "trocr_pilot_summary.csv"),
