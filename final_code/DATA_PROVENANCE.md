...
`final_code/preprocessing/normal/Iam_split.py` only *reads* pre-computed IAM splits
...
`final_code/data/normal/` are ever lost or corrupted, this project cannot regenerate them on its own - the
...
this repo at `final_code/preprocessing/normal/generate_trocr_split.py`, with only two path-level
...
since `train_model/normal/trocr/TROcrFineTune.py` already covers that, adapted to local paths.
...
`final_code/preprocessing/normal/Iam_split.py` 只负责**读取**预先算好的IAM切分（`data/normal/crnn/{train,val,test}.csv`、`data/normal/trocr/{train,val}.csv`），从来不会从头生成它们。真正的生成过程是在这个repo之外、独立的Google Colab notebook里完成的。这份文档记录哪些notebook已经找到并核对过、哪些还没找到。
